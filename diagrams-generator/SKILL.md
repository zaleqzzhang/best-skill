...
| **HTML 方案图** (手绘卡片/Infographic) | `HTML + Playwright` | ⚡ 中 | ★★★★★ | 中量 (~150MB) |
...
2. **Identify Scenario**: 根据用户描述中的关键词，识别场景类型：
   ...
   | 网络/拓扑/节点/关系 | → 关系图 |
   | 论文/学术/神经网络/模型架构 | → 学术图 |
   | 方案图/HTML/infographic/PRD配图/文章插图/手绘卡片/海报图 | → HTML 方案图 |
...
4. **⚠️ MANDATORY OUTPUT - 输出以下结构化内容**:
   ...
   **场景: HTML 方案图 (手绘卡片 / 技术方案 / Infographic)**
   
   | 方案 | 工具 | 速度 | 质量 | 输出格式 | 说明 |
   |------|------|------|------|----------|------|
   | **A** | `HTML + Tailwind CDN + Playwright` | ⚡ 中 | ★★★★★ | HTML/PNG | 推荐，CSS 完全可控，emoji/中文完美 |
   | **B** | `HTML + 纯 CSS + Playwright` | ⚡ 中 | ★★★★ | HTML/PNG | 离线可用，无 CDN 依赖 |
   | **C** | `HTML + Tailwind + html2image` | ⚡ 快 | ★★★★ | HTML/PNG | 轻量，依赖系统 Chrome |
   
   **💡 推荐**: 方案 A (`Playwright`) - 还原度最高
   
   **🎨 风格选项 (在选定方案后，请用户从中选 1)**：
   
   | 风格 | 标识 | 视觉特征 | 适用场景 |
   |------|------|---------|---------|
   | **1** | 🎨 手绘卡片风 (Morandi) | 米色底 + 莫兰迪暖卡片 + emoji 步骤 + 圆角虚线 | 技术方案/PRD 配图（默认） |
   | **2** | 🌙 现代深色风 (Dark Tech) | 深色底 + 霓虹高亮 + 网格背景 | 系统架构/Hero 配图 |
   | **3** | 📐 简约白板风 (Whiteboard) | 纯白 + 细线框 + 单色强调 | 论文/正式文档 |
   | **4** | 🌈 渐变科技风 (Gradient Hero) | 紫蓝渐变 + 毛玻璃卡片 | 产品发布/营销 |
   | **5** | 📰 杂志风 (Editorial) | 大标题 + 重点数字 + 强对比 | 公众号/数据洞察 |
   
   完整模板见 [references/html-templates.md](references/html-templates.md)
...
#### HTML 方案图 工作流（场景 = HTML 方案图）

如果用户选择了 HTML 方案图场景，按以下步骤生成（与 Python 绘图分支并列）：
...
playwright install chromium  # ~150MB，1-2 分钟
```

如果依赖未装，render.py 会捕获 ImportError 并打印安装命令，不要直接 fail。

**2. 选择风格模板**

参考 [references/html-templates.md](references/html-templates.md)，5 种风格备选（见 Phase 1 风格选项表）。
模板 1 是完整可运行 HTML，模板 2-5 是基于模板 1 的"配色 + 关键 CSS 覆盖"差异块。

**3. 产物结构**

```
./pic/{name}/
  ├── {name}.html        # HTML 源码（可二次手改）
  ├── render.py          # Playwright 截图脚本
  ├── server.py          # 本地 HTTP 服务（供 preview_url 使用）
  └── {name}.png         # 渲染产物（device_scale_factor=2，Retina）
```

**4. 渲染脚本 (render.py) 通用模板**

```python
import asyncio, os, sys

OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))
HTML_FILE  = os.path.join(OUTPUT_DIR, "{name}.html")
...
async def render():
    ...
    async with async_playwright() as p:
        ...
        await page.wait_for_load_state("networkidle")  # 等 Tailwind CDN 加载
...
1. 写入 3 个文件：`{name}.html` / `render.py` / `server.py`
2. 执行 `python render.py` → 生成 PNG
3. **后台启动** `python server.py`（不要阻塞），从 stdout 读取实际端口
4. **调用 `preview_url`** 工具打开浏览器预览（这是 HTML 场景独有，**不要用 `open_result_view`**）
5. 报告产物路径与预览链接
