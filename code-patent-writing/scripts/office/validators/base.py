...
class BaseSchemaValidator:
    ...
    def repair(self) -> int:
        ...
        repairs += self.repair_unreferenced_media()
        return repairs

    def repair_unreferenced_media(self) -> int:
        """Remove unreferenced files from word/media/ (or ppt/media/, xl/media/).

        Template placeholder images that were orphaned during content replacement
        are harmless but cause ``validate_file_references()`` to report CRITICAL
        errors.  Deleting them in the *repair* phase keeps validation clean.
        """
        import lxml.etree as _ET

        media_dirs = [
            d for d in self.unpacked_dir.iterdir()
            if d.is_dir() and d.name in self.MAIN_CONTENT_FOLDERS
        ]
        media_folders: list[Path] = []
        for top in media_dirs:
            mf = top / "media"
            if mf.is_dir():
                media_folders.append(mf)
        if not media_folders:
            return 0

        # Collect all files referenced by any .rels
        all_referenced: set[Path] = set()
        for rels_file in self.unpacked_dir.rglob("*.rels"):
            try:
                rels_root = _ET.parse(str(rels_file)).getroot()
        ...
        for mf in media_folders:
            for media_file in sorted(mf.iterdir()):
                if not media_file.is_file():
                    continue
                if media_file.resolve() not in all_referenced:
                    rel_path = media_file.relative_to(self.unpacked_dir)
                    print(f"  Repaired: removed unreferenced media file {rel_path}")
                    media_file.unlink()
                    repairs += 1
        return repairs
